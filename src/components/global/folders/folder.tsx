...
import { renamedFolders } from '@/actions/workspace'
import { useMutationData, useMutationDataState } from '@/hooks/useMutuationData'
...
//   const {mutate,isPending}=useMutationData(["rename-folders"],(data:{name:string})=>renamedFolders(id,data.name),"workspace-folders",Renamed)
...
//           {onRename ? <Input onBlur={(e:React.FocusEvent<HTMLInputElement>)=>updateFolderName(e)} autoFocus ref={inputRef} placeholder={name} className='border-none  text-base outline-none text-neutral-300 bg-transparent p-0'/>:( <p onClick={(e)=>{e.stopPropagation()}} onDoubleClick={handleNameDoubleClick} className='text-neutral-300'>{name}</p>)}
